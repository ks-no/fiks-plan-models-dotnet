pipeline {
    ...
    parameters {
        ...
        string(name: "triggerbranch", defaultValue: "main", description: "Hvilke branch fra KS.Fiks.Plan.Specification skal det bygges fra?" )
    }
    environment {
        MODELS_FOLDER = 'KS.Fiks.Plan.Models.V2'
        GENERATOR_FOLDER = 'KS.Fiks.Plan.JsonModelGenerator'
    }
    stages {
        stage('Initialize') {
            steps {
        ...
        stage('Fetch specification') {
          steps { 
            dir('temp') {
              ...
              stash(name: 'xsd', includes: 'Schema/V2/*')
